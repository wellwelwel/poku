...
- If you request an AI to write everything, we'll probably use another AI to "read" it when it's convenient, and we probably won't read what you didn't write too. We hope you understand 🤝
...
- Also be careful when changing existing tests. It usually means a breaking change or that something that used to work no longer does, so prefer adding new tests and only change an existing one when there's really no other way _(an intentional breaking change, a test that was genuinely wrong, updated snapshots, etc.)_.

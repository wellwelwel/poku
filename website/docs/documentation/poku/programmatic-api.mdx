...
:::note Advanced
This is a low-level API. Prefer the [**CLI**](/docs/documentation/poku/include-files#cli) or a [**`poku.config.js`**](/docs/documentation/poku/config-files) for almost everything. To orchestrate containers or services, use a [plugin](/docs/documentation/plugins/creating-plugins) with `setup`/`teardown`. To run multiple suites, use [`@pokujs/multi-suite`](/docs/documentation/plugins/official/multi-suite). Reach for the programmatic API only when you need imperative control over the run, such as conditional multi-phase flows.
:::
...
:::tip
For most multi-suite needs, prefer the config-based approach with [`@pokujs/multi-suite`](/docs/documentation/plugins/official/multi-suite), which runs independent suites sequentially from a single and simple `poku.config.js`.
:::

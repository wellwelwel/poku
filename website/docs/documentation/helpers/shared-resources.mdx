...
When you call `getSharedResource` , it returns a tuple with the resource and a `detach` function. This function should be called to
detach the test from the resource, allowing cleanup and preventing memory leaks:

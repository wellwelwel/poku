...
test('increments the counter', async () => {
  const counter = await getSharedResource('sharedCounter');
...
test('can share cache', async () => {
  const cache = await getSharedResource('lru');
...
export default createSharedResource('mysql', async () => {
...
test('can query users', async () => {
  const db = await getSharedResource('mysql');

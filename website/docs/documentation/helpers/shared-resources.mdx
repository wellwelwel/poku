...
test('increments the counter', async () => {
  const [counter, dispose] = await getSharedResource('sharedCounter');
...
### 4. Dispose of Shared Resources

When you're done with a shared resource, call the `dispose` function returned by `getSharedResource` to clean up:

```ts
test('disposes of the resource', async () => {
  const [counter, dispose] = await getSharedResource('sharedCounter');
  await counter.increment();
  assert.equal(await counter.getCount(), 1);
  dispose(); // Clean up the resource
});
```
...
test('can share cache', async () => {
  const [cache, dispose] = await getSharedResource('lru');
...
test('can query users', async () => {
  const [db, dispose] = await getSharedResource('mysql');

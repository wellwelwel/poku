...
Release a leaking port with **Poku**'s [`kill`](/docs/documentation/helpers/processes/kill) helper:

```ts
import { kill } from 'poku';

await kill.port(3000);
```

- Replace `3000` with the port you need to release.

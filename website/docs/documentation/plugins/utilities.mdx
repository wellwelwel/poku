...
An identity function that provides type checking and IDE autocompletion when creating plugins. Returns the plugin object as-is, with no runtime transformation.
...
When `skipInternal` is `true`, lines containing `poku/lib/` are skipped, which is useful for finding the caller's file instead of **Poku** internals.

...
```ts
strict(true, 'Isso é válido 🧪');
strict.strictEqual(1, '1', 'O Poku irá descrever isso e mostrar um erro 🐷');
// ...
```

:::tip
Os métodos `assert` e `strict` do **Poku** usarão a mensagem exatamente como seria ao usar `describe` e `it`. <br />

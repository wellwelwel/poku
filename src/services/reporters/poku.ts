...
const emptyOptions = Object.create(null) as DescribeOptions;

export const poku: ReturnType<ReporterPlugin> = {
  onRunStart() {},
  onFileStart() {},
  onDescribeAsTitle(title, options) {
    const { background, icon } = options ?? emptyOptions;
  ...
  onAssertionFailure({ assertOptions: options, error }) {
    ...
    const absolutePath = fileRef.startsWith('file://')
      ? fileRef.slice(7)
      : fileRef;
  ...
  onRunResult() {
    ...
    hr();
    log(
      `${format(String(length)).fail().bold()} ${format('test file(s) failed:').bold()}\n`
    );
